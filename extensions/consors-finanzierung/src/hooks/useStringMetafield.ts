...
>( 
  ...
  const [metafieldState,setMetafieldState] = useState<typeof metafield["value"]>()
  const [metafieldWriteInProgress,setMetafieldWriteInProgress] = useState(false)
  ...
  return [metafield?.value, update] as [string, (newValue: string) => Promise<MetafieldChangeResult>]
}
