...
export function createShopifyOrderCancelUnhandled(
  shop: string,
  orderId: number,
...
export function incrementCounterShopifyOrderCancelUnhandled(orderId: bigint) {
  return db.shopifyOrderCancelUnhandled
    .findUnique({ where: { orderId } })
    .then((order) => {
      if (order != undefined) {
        return db.shopifyOrderCancelUnhandled.update({
          where: { orderId },
...
export async function handleShopifyOrderCancel(orderId: bigint) {
  const order = await db.shopifyOrderCancelUnhandled.findUnique({
    where: { orderId },
  });
  if (order == null) {
    return false;
  ...
  await db.shopifyOrderCancelHandled
    .create({
      data: order,
    })
