...
export function getCheckoutState(checkout:  Pick<NonNullable<Awaited<ReturnType<typeof getCheckout>>>,"uuid">){
  return db.checkoutState.findMany({where:{checkoutId: checkout.uuid}, orderBy:{createdAt: "desc"}})
}

export function createCheckoutState(checkout:  Pick<NonNullable<Awaited<ReturnType<typeof getCheckout>>>,"uuid">, state: string, notifyRequest: string, creditAmount: number|undefined){
  return db.checkoutState.create({data:{checkoutId: checkout.uuid, state, notifyRequest, creditAmount: creditAmount==undefined ? undefined : creditAmount?.toFixed(2) }})
}
...
export async function getOrCreateCheckout(shop: string, checkoutToken: string) {
  ...
  //  return res
  //}
  return db.checkout.create({data:{uuid: checkoutToken, shop}})
