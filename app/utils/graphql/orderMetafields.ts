...
export async function getTransactionId(shop:string, admin_graphql_api_id:string) {
  ...
  export async function getPaymentState(shop:string, admin_graphql_api_id:string) {
    const gqlClient = await getGraphqlClient(shop)
    const query = `{
      order(    id: "${admin_graphql_api_id}") {
  ...
  export async function getConsorsused(shop:string, admin_graphql_api_id:string) {
    const gqlClient = await getGraphqlClient(shop)
    const query = `{
      order(    id: "${admin_graphql_api_id}") {
